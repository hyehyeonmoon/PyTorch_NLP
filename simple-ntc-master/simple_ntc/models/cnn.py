...
class CNNClassifier(nn.Module):
    ...
    def __init__(
        ...
        self.feature_extractors = nn.ModuleList() #Module을 담는 list
    ...
    def forward(self, x):
        ...
        # 만약 입력의 length가 window size보다 작다면 padding을 넣어줘서 오류를 피해야 함(아래코드)
        ...
        if min_length > x.size(1):
            ...
            # |pad| = (batch_size, min_length - length, word_vec_size) #부족한 부분만큼 min_length-length 채워줌
            x = torch.cat([x, pad], dim=1)
            # |x| = (batch_size, min_length, word_vec_size)
        
        # 이미지 task에 맞게 3차원 tensor를 4차원 tensor로 만들어주기
        ...
        for block in self.feature_extractors:
            ...
            #max pooling은 가중치가 필요없기 때문에 함수로 선언해줌 
            #max pooling의 결과는 가변에서 고정길이가 됨.
            cnn_out = nn.functional.max_pool1d( 
                input=cnn_out.squeeze(-1), # (bs, NF, 1)
                kernel_size=cnn_out.size(-2) # (length-window_size+1)
            ).squeeze(-1)
            # |cnn_out| = (batch_size, n_filter, 1)
            #.squueze(-1)을 하게 되면
            # |cnn_out| = (batch_size, n_filter)
            cnn_outs += [cnn_out] 
            
        # Merge output tensors from each convolution layer.
        cnn_outs = torch.cat(cnn_outs, dim=-1) #행을 기준으로 붙임
        # |cnn_outs| = (batch_size, sum(n_filters)) <=> 문장 임베딩 벡터, 문장은 batch_size 개수만큼 있음
